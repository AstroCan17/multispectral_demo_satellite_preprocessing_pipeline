...
class PanSharpening:
    ...
    def pan_sharpen(self,rgb_img,pan_img,save_path):
        ...
        # root.destroy()
        if not os.path.exists(f"{save_path}/04_pansharp/16b_images"):
            os.makedirs(f"{save_path}/04_pansharp/16b_images")
            cv2.imwrite(f"{save_path}/04_pansharp/16b_images/pan_sharpened.tif", sharpened_image)

        if not os.path.exists(f"{save_path}/04_pansharp/8b_images"):
            os.makedirs(f"{save_path}/04_pansharp/8b_images")
            cv2.imwrite(f"{save_path}/04_pansharp/8b_images/pan_sharpened_8b.tif", sharp_8b)
...
def run_pansharp(save_path):
    ...
    rgb_path = f"{save_path}/03_rgb/16b_images/colombus_registered_rgb_12b.tif"
    pan_path = f"{save_path}/02_denoised/16b_images/colombus_wiener_deconv_b6.tif"
    
    # rgb_path = r"D:\03_cdk_processing\03_IPS_pipeline\01_reprocess\06_anitkabir\02_implement\05\16b_images\colombus_registered_rgb_12b.tif"
    # pan_path = r"D:\03_cdk_processing\03_IPS_pipeline\01_reprocess\06_anitkabir\01_cut\colombus_nuc_band_b6.tif"
