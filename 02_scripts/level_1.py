...
class NUC:
    ...
    def apply_nuc_and_bpr(self,orbit_generator,gain_dict,offset_dict,common_band_list,bad_pixels,save_img = False):
        ...
        for i,(name,img) in enumerate(orbit_dict.items()):
            ...
            if save_img == True:
                # cv2.imwrite(f"{save_path}/colombus_nuc_band_b{name[-1]}.tif", NUC.orientation_tek(self,nuc_bpr_frame))
                print("Saving the NUC band {}...".format(name[-1]))
                cv2.imwrite(f"{save_path}/colombus_nuc_band_b{name[-1]}.tif", nuc_bpr_frame)
            #     cv2.imwrite(f"{save_path}/16b_images/colombus_nuc_band_{name[-1]}.tif", NUC.orientation_tek(self,nuc_bpr_frame))
            #     cv2.imwrite(f"{save_path}/16b_images/colombus_raw_band_{name[-1]}.tif", NUC.orientation_tek(self,img))
            ...
            #     cv2.imwrite(f"{save_path}/8b_images/colombus_nuc_band_{name[-1]}_8b.tif", NUC.orientation_tek(self,nuc_img_8b))
            #     cv2.imwrite(f"{save_path}/8b_images/colombus_raw_band_{name[-1]}_8b.tif", NUC.orientation_tek(self,raw_img_8b))
            # else:
            #     pass
...
class SatelliteAttitude:
    ...
    def get_tle(self):
        # Colombus_T2 = tlefile.read(self.satellite)
        # Line1 = Colombus_T2.line1
        # Line2 = Colombus_T2.line2
...
class sharpening:
    ...
    def deconvolution_kernel(self, input_generator,save_path,save_img = False):
        ...
        for key, img in input_dict.items():
            ...
            # sharped_dict[key] = img
            img = NUC.orientation_tek(self,img)
            if save_img:
                
                print("Saving the sharpened images...")
                # cv2.imwrite(f"{save_path}/16b_images/colombus_sharpened_band_{key[-1]}.tif", img)
                cv2.imwrite(f"{save_path}/02_denoised/16b_images/colombus_wiener_deconv_b{key[-1]}.tif", img)
                img_8b = (((img - np.min(img)) / (np.max(img) - np.min(img)))*255).astype(np.uint8)
                # cv2.imwrite(f"{save_path}/8b_images/colombus_sharpened_band_{key[-1]}_8b.tif", img_8b)
                cv2.imwrite(f"{save_path}/02_denoised/8b_images/colombus_wiener_deconv_b{key[-1]}_8b.tif", img_8b)
            else:
                pass
...
class Denoiser:
    ...
    def pca(self,input_generator,components = "",save_img = False):
        ...
        for key, img in input_dict.items():
            ...
            if save_img:
                cv2.imwrite(f"{save_path}/16b_images/colombus_pca_denoised_band_{key[-1]}.tif", NUC.orientation_tek(self,img_inverted))
                img_8b = (((img_inverted - np.min(img_inverted)) / (np.max(img_inverted) - np.min(img_inverted)))*255).astype(np.uint8)
                cv2.imwrite(f"{save_path}/8b_images/colombus_pca_denoised_band_{key[-1]}_8b.tif", NUC.orientation_tek(self,img_8b))
    ...
    def moving_avarage_filter(self, input_generator, save_img=False):
        ...
        for key, img in input_dict.items():
            ...
            if save_img:
                ...
                cv2.imwrite(f"{save_path}/16b_images/colombus_denoised_band_{key[-1]}.tif", NUC.orientation_tek(self,filt_sig))
                img_8b = (((filt_sig - np.min(filt_sig)) / (np.max(filt_sig) - np.min(filt_sig)))*255).astype(np.uint8)
                cv2.imwrite(f"{save_path}/8b_images/colombus_denoised_band_{key[-1]}_8b.tif", NUC.orientation_tek(self,img_8b))
    ...
    def wavelet_denoising_cdk(self, input_generator,save_img = False):
        ...
        for key, value in input_dict.items():
            sigma_est = estimate_sigma(value, channel_axis=0, average_sigmas=True)
            print(f'Estimated Gaussian noise standard deviation = {sigma_est}')
            im_visushrink = denoise_wavelet(value,sigma=sigma_est/3, wavelet='db3', mode='soft',
                                            wavelet_levels=20, convert2ycbcr=False, 
                                            method='VisuShrink', rescale_sigma=True, 
                                            channel_axis=None)

            im_visushrink = im_visushrink.clip(0, 2**12-1).astype(np.uint16)

            
            if save_img == True:
                cv2.imwrite(f"{save_path}/16b_images/colombus_denoised_band_{key[-1]}.tif", NUC.orientation_tek(self,im_visushrink))
                im_visushrink_8b = (((im_visushrink - np.min(im_visushrink)) / (np.max(im_visushrink) - np.min(im_visushrink)))*255).astype(np.uint8)
                cv2.imwrite(f"{save_path}/8b_images/colombus_denoised_band_{key[-1]}_8b.tif", NUC.orientation_tek(self,im_visushrink_8b))
            else:
                pass
            yield "dNoised_"+key, im_visushrink
    ...
    def dark_noise_removal(self, orbit_generator, dark_generator,save_img = False):
        if save_img == True:
            save_path = self.save_to_folder()
            if not os.path.exists(f"{save_path}/16b_images"):
                os.makedirs(f"{save_path}/16b_images")
            if not os.path.exists(f"{save_path}/8b_images"):
                os.makedirs(f"{save_path}/8b_images")
            print(" noise free images are saving to: ", save_path)
        print("Dark noise removal activated...")
        if isinstance(orbit_generator, dict):
            orbit_dict = orbit_generator
        else:
            orbit_dict = dict(orbit_generator)
        if isinstance(dark_generator, dict):
            dark_dict = dark_generator
        else:
            dark_dict = dict(dark_generator)

        for key, value in orbit_dict.items():
            for dark_key in dark_dict.keys():
                if key[-2:] == dark_key[-2:]:
                    dark = dark_dict[dark_key]
                    dark_cut = dark[0: value.shape[0], 0: value.shape[1]]
                    dark_fft = np.fft.fft2(dark_cut)
                    value_fft = np.fft.fft2(value)
                    value_fft = np.subtract(value_fft, dark_fft)
                    

                    denoised_band = np.fft.ifft2(value_fft)
                    # keep real part of the inverse fft
                    denoised_band = np.real(denoised_band)

                    # denoised_band = np.abs(denoised_band)
                    
                    denoised_band = denoised_band.clip(0, 2 ** 12 - 1).astype(np.uint16)
                    

                    print(f"Dark noise removed from band_{key[-1]}")
                    yield (f"darkFree_b{key[-1]}", denoised_band)
                    if save_img:
                        cv2.imwrite(f"{save_path}/16b_images/colombus_darkFree_band_{key[-1]}.tif", denoised_band)
                        denoised_band_8b = (((denoised_band - np.min(denoised_band)) / (np.max(denoised_band) - np.min(denoised_band)))*255).astype(np.uint8)
                        cv2.imwrite(f"{save_path}/8b_images/colombus_darkFree_band_{key[-1]}_8b.tif", denoised_band_8b)
    ...
    def gaussian_filter_ips(self, input_generator,save_img = False):
        ...
        for key, value in input_dict.items():
            ...
            if save_img:
                cv2.imwrite(f"{save_path}/16b_images/colombus_gaussian_filtered_band_{key[-1]}.tif", NUC.orientation_tek(self,value))
                value_8b = (((value - np.min(value)) / (np.max(value) - np.min(value)))*255).astype(np.uint8)
                cv2.imwrite(f"{save_path}/8b_images/colombus_gaussian_filtered_band_{key[-1]}_8b.tif", NUC.orientation_tek(self,value_8b))
            else:
                pass
    ...
    def get_filtered_butterworth(self,image_generator,cutoff_inp, squared_butterworth=True, order=3.0, npad=0, save_img=False):
        ...
        # keys_list = list(key[-2:] for key in image_dict.keys())


        # def cutoff_freq_estimate(keys_list):
        #     dark_path = r"D:\03_cdk_processing\04_github\04_deneme\01_dark"
        #     band_list = ['b2', 'b3', 'b4', 'b6']
        #     cut_off_freq_dict = {}
        #     for filename in os.listdir(dark_path):
        #         name = filename.split(".")[0]
        #         if name[-2:] in keys_list:
        #             dark_img = cv2.imread(os.path.join(dark_path, filename), cv2.IMREAD_UNCHANGED)
        #             mean = np.mean(dark_img)
        #             sigma = np.std(dark_img)
        #             snr = mean/sigma
        #             cut_off_freq_dict[name[-2:]] = snr
        #             print(f"SNR of band_{name[-2:]} is {snr}")
        #     return cut_off_freq_dict

        # cut_off_freq_dict = cutoff_freq_estimate(keys_list)
        ...
        for name, image in image_dict.items():
            ...
            if save_img:
                cv2.imwrite(f"{save_path}/16b_images/colombus_butterworth_filtered_band_{name[-1]}.tif", denoised_img)
                denoised_img_8b = (((denoised_img - np.min(denoised_img)) / (np.max(denoised_img) - np.min(denoised_img)))*255).astype(np.uint8)
                cv2.imwrite(f"{save_path}/8b_images/colombus_butterworth_filtered_band_8b_{name[-1]}.tif", denoised_img_8b)
    ...
    def butterworth_LP_ips(self,img_generator):
        ...
        for name, img in image_dict.items():
            ...
            # noise_sigma = np.std(center_shift[:, center[1]-5:center[1]+5])
    ...
    def bos_none(self,img_generator):
        if isinstance(img_generator, dict):
            image_dict = img_generator
        else:
            image_dict = dict(img_generator)
        ...
        for name, img in image_dict.items():
            # print(f"bos_none_b{name[-1]}")
            yield f"bos_none_b{name[-1]}", img
